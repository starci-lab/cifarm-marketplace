...
export type WriteResponse = Response<WriteResponseData>;
...
export class StorageService {
    ...
    constructor() {
        this.apiUrl = join(envConfig().cibase.apiUrl, "storage")
    }

    async write(body: WriteRequestBody): Promise<WriteResponse> {
        const { data } = await authAxios.post<WriteResponse>(
            this.apiUrl,
            body
        )
        return data
